...
def main() -> None:
    ...
    # If --out is not a .csv, treat it as a directory and create a timestamped CSV
    ...
    # ---- config template (seed varies per rep) ----
    cfg_template = RiskQAEConfig(
        ...
        budget=BudgetConfig(
            ...
            seed=args.seed,  # overridden per rep
        ),
    ...
    # ---- classical on discretized (fixed for all reps) ----
    ...
    for rep in range(int(args.reps)):
        ...
        # ---- quantum ----
        res = estimate_risk_measures({"samples": samples}, alphas=alphas, config=cfg)
        ...
        # One row per alpha (best for analysis later)
        for a in alphas:
            vr = res.var[a]
            tr = res.tvar[a]
